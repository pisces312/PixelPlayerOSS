...
class ChangelogParserTest {
    ...
    fun parse_readsShippedChangelogVersionsInOrder() {
        ...
        assertEquals(listOf("0.3.0", "0.2.0", "0.1.0"), parsed.map { it.version })
        assertEquals(listOf("2026-08-15", "2026-07-17", "2026-06-09"), parsed.map { it.date })
    }
    ...
    fun parse_readsEverySectionOfTheShippedReleases() {
        ...
        // 0.3.0 and 0.1.0 both declare five Keep a Changelog headings, including the
        // 0.1.0-only "App polish included in this FOSS release" section.
        assertEquals(listOf(5, 4, 5), parsed.map { it.sections.size })
        parsed.forEach { version ->
