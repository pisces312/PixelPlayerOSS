...
| Asset | Upstream | License |
| --- | --- | --- |
| `app/src/main/assets/cities.json` (China) | AreaCity-JsSpider-StatsGov, `https://github.com/xiangyuecn/AreaCity-JsSpider-StatsGov` | MIT |
| `app/src/main/assets/cities.json` (rest of the world) | GeoNames `cities15000`, `https://download.geonames.org/export/dump/` | Creative Commons Attribution 4.0 (CC-BY 4.0) |

`cities.json` is generated by `gen_city_catalog.py`. It backs the offline place picker and the
offline "coordinates to place name" lookup in Serendipity, so those features need no geocoding
service and send no device position to a third party. It holds every Chinese province, city and
district, plus cities of 100k+ people elsewhere.
